...
*/
	...
	class Innoexts_Warehouse_Model_Mysql4_Catalog_Product_Indexer_Price_Configurable extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Indexer_Price_Configurable {
		/**
     ...
     */
		function _prepareFinalPriceData($entityIds = null) {
			...
			$write->quoteInto( '=?', STATUS_ENABLED );
			...
			$query = $statusCond = $select->insertFromSelect( $this->_getDefaultFinalPriceTable(  ) );
			$write->query( $query );
     ...
     */
		function _applyConfigurableOption() {
			$indexerHelper = $this->getProductPriceIndexerHelper(  );
			$write = $this->_getWriteAdapter(  );
			$coaTable = $this->_getConfigurableOptionAggregateTable(  );
			$finalPriceTable = $copTable = $this->_getConfigurableOptionPriceTable(  );
			...
			$write->query( $query );
			$table = array( 'i' => $finalPriceTable );
			$indexerHelper->getConfigurableOptionFinalPriceSelect( $write, $copTable );
			$select = $this->_getDefaultFinalPriceTable(  );
			$select->crossUpdateFromSelect( $table );
			$query = $this->_prepareConfigurableOptionAggregateTable(  );
			$write->query( $query );
