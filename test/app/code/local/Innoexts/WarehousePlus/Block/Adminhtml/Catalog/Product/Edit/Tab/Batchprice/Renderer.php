...
*/
	...
	class Innoexts_WarehousePlus_Block_Adminhtml_Catalog_Product_Edit_Tab_Batchprice_Renderer extends Innoexts_Warehouse_Block_Adminhtml_Catalog_Product_Edit_Tab_Batchprice_Renderer {
		/**
     * Constructor
     */
		function __construct() {
			$this->setTemplate( 'warehouseplus/catalog/product/edit/tab/batchprice/renderer.phtml' );
		}
			...
			if (( count( $currencies ) && count( $stockIds ) )) {
				...
				foreach ($currencies as $currency) {
					...
					foreach ($stockIds as $stockId) {
						$value = array( 'currency' => $currency, 'stock_id' => $stockId );
						$defaultPrice = $priceHelper->getDefaultPrice2( $product, $storeId, $currency, $stockId );
